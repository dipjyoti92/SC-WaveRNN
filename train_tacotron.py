...
def tts_train_loop(model, optimizer, train_set, lr, train_steps, attn_example):
    ...
    for e in range(epochs):
        ...
        for i, (x, m, ids, _, s_e) in enumerate(train_set, 1):

            optimizer.zero_grad()

            x, m, s_e = x.cuda(), m.cuda(), s_e.cuda()

            m1_hat, m2_hat, attention = model(x, m, s_e)
...
def create_gta_features(model, train_set, save_path):
    ...
    for i, (x, mels, ids, mel_lens, s_e) in enumerate(train_set, 1):

        x, mels, s_e = x.cuda(), mels.cuda(), s_e.cuda()

        with torch.no_grad() : _, gta, _ = model(x, mels, s_e)
